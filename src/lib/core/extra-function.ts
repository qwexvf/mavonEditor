 ...
 * textarea 插入内容
 */
export const insertTextAtCaret = (obj: any, { prefix, subfix, str, type }: any, $vm: any) => {
    obj.focus()
...
// 删除行
export const removeLine = ($vm: any) => {
    let obj = $vm.getTextareaDom();
    if (typeof obj.selectionStart === 'number' && typeof obj.selectionEnd === 'number') {
        var startPos = obj.selectionStart;
        var endPos = obj.selectionEnd;
...
// 插入tab
export const insertTab = ($vm: any, tab: any) => {
...
// shift + tab
export const unInsertTab = ($vm: any, tab: any) => {
...
// 插入enter
export const insertEnter = ($vm: any, event: any) => {
    let obj = $vm.getTextareaDom()
    if (typeof obj.selectionStart === 'number' && typeof obj.selectionEnd === 'number') {
        let startPos = obj.selectionStart;
        let endPos = obj.selectionEnd;
 ...
 * 生成导航目录
 */
export const getNavigation = ($vm: any, full: any) => {
    let navigationContent = $vm.$refs.navigationContent
    ...
    function judageH(node: any, i: any, nodes: any) {
        let reg = /^H[1-6]{1}$/;
 ...
 * 滚动条联动
 */
export const scrollLink = ($event: any, $vm: any) => {
 ...
 * @param $vm
 */
export const fullscreenchange = ($vm: any) => {
    // 阅读模式 全屏监听事件
 ...
 * @param $vm
 */
export const windowResize = ($vm: MarkdownEditor) => {
...
export function loadScript(src: string, callback: CallableFunction) {
...
export function loadLink(src: string, callback: CallableFunction) {
    if (!(typeof callback === 'function')) {
        callback = function () { };
    }
...
export const ImagePreviewListener = ($vm: Component) => {
    // vShowContent
